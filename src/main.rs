...
mod fs { 
    vulkano_shaders::shader! {
        ...
        src: r"
            ...
            void main() {
                vec3 col = 0.5 + 0.5*cos(PushConstants.time/100.0+frag_coord.xyx+vec3(0,2,4));
...
struct PushConstants { //small data sent to GPU during a draw command, immutably read from shaders
    time : f32
}
...
impl Application {
        ...
        let new_command_buffers : Vec<Arc<PrimaryAutoCommandBuffer>> = framebuffers
            ...
            .map(|framebuffer| { 
                ...
                builder 
                    ...
                    .unwrap()
                    .push_constants(graphic_pipeline_layout.clone(), 0, PushConstants{time: self.current_time}) //pass the new frame's time value
                    .unwrap()
    ...
    fn submit_drawcall(&mut self) {
        ...
        *thing = Some(Arc::new(execution.unwrap())); //store the execution, to be synchronized to later
        self.current_time = self.current_time + 1.0; //a very bad way of doing time, but it works for my purpose
    }
